...
public class PortalDataMarshallerTest extends AbstractMarshallerTest
{
   ...
   public void testPortalDataUnmarshalling()
   {
      ...
      ContainerData layout = data.getPortalLayout();
      ...
      assertNotNull(layout.getAccessPermissions());
      assertTrue(layout.getAccessPermissions().isEmpty());
      List<ComponentData> children = data.getPortalLayout().getChildren();
      assertEquals(5, children.size());
      int bodyCount = 0;
      for (ComponentData component : children)
      {
         if (component instanceof ApplicationData)
         {
         }
         else if (component instanceof BodyData)
         {
            bodyCount++;
         }
         else
      ...
      // Verify banner portlet app
      {
         ...
         ApplicationData<Portlet> application = (ApplicationData<Portlet>) children.get(0);
      ...
      {
         @SuppressWarnings("unchecked")
         ApplicationData<Portlet> application = (ApplicationData<Portlet>) children.get(1);
         assertTrue(application.getType() == ApplicationType.PORTLET);
         ApplicationState<Portlet> state = application.getState();
         assertNotNull(state);
         assertTrue(state instanceof TransientApplicationState);
